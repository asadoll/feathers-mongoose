...
class MongooseAdapter extends AdapterBase {
  ...
  $find (params = {}) {
    ...
    // Handle $populate
    if (filters.$populate && this.options.allow.includes('$populate')) {
  ...
  $create (_data, params = {}) {
    ...
    return model.create(data, params.mongoose).then(results => {
      ...
      if ($populate && this.options.allow.includes('$populate')) {
    ...
    try {
      return ids.then(idList => {
        ...
        const findParams = Object.assign({}, params, {
          paginate: false,
          query: $populate && this.options.allow.includes('$populate') ? Object.assign(updatedQuery, { $populate }) : updatedQuery
        });
